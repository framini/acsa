                                ...
                                <tbody>
                                        <?php 
                                        	if( !is_null($productos) ) { 
                                                ...
                                                {
                                                    echo "<tr id=producto-" . $producto['cuenta_corriente_id'];
													echo ">";
                                                    echo "<td class='id-producto'>" . $producto['cuenta_corriente_id'] .'</td>';
                                                    echo "<td class='nombre-producto'>";
													echo $producto['nombre'] .'</td>';
													echo "<td class='owner-producto'>";
                                                   ...
                                                   if(count($data_menu) > 0) {
	                                                    ...
	                                                    foreach ($data_menu as $keyp => $row) { 
								               				if(!$row['boton_superior']) {
								               					...
								               					echo anchor($this->uri->segment(1) . "/" . $this->uri->segment(2) ."/". $keyp . '/' .$producto['cuenta_corriente_id'], '<i class="' . $row['icono'] . '"></i> ' . $row['texto_anchor'], $atributos);

...
class Adm_Personas extends MY_Controller {
	...
	function registro_empresa() {
		...
		if ($this -> auth_frr -> es_admin()) {
			...
			if ($this -> form_validation -> run()) {
				if (!is_null($data = $this -> empresas_frr -> create_empresa($this -> form_validation -> set_value('nombre'), $this -> form_validation -> set_value('cuit'), $this -> form_validation -> set_value('tipo_empresa_id')))) {
	...
	function modificar_empresa() {
		...
		if ($this -> auth_frr -> es_admin()) {
			...
			//Chequeamos que los datos enviados por formulario sean correctos
			if ($this -> form_validation -> run()) {
				if (!is_null($data = $this -> empresas_frr -> modificar_empresa($this -> form_validation -> set_value('nombre'), $this -> form_validation -> set_value('cuit'), $this -> form_validation -> set_value('tipo_empresa_id'), $this -> uri -> segment(4)))) {
					//Nos fijamos si la petición se hizo via AJAX
					if ($this -> input -> is_ajax_request()) {
	...
	function registro_cuenta_registro() {
		...
		if ( $this -> auth_frr -> es_admin() || $this-> auth_frr -> is_warrantera() ) {
			...
			if ($this -> form_validation -> run()) {
				...
				if (!is_null($data = $this -> empresas_frr -> create_cuenta_registro($this -> form_validation -> set_value('nombre'), $this -> form_validation -> set_value('codigo'), $this -> form_validation -> set_value('tipo_cuentaregistro_id'), $this -> form_validation -> set_value('empresa_id')))) {
